...
class PyCFGTester(TestCase):
    def test_multi_class_py(self):
        argv = sys.argv
        sys.argv = sys.argv+['--cfg', 'cfgs/py/train/classify/multi_class.py', 'train.train_steps=400', 'train.train_epochs=null']
        neko_train()
        sys.argv = argv

    def test_multi_class_workflow_eval(self):
        argv = sys.argv
        sys.argv = sys.argv+['--cfg', 'cfgs/py/train/classify/multi_class_floweval.py', 'train.train_steps=400', 'train.train_epochs=null']
        neko_train()
        sys.argv = argv
