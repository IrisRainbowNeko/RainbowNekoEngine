...
class StyleLoss(LossContainer):
    ...
    def forward(self, pred, target):
        ...
        if 2 in target_tensor:  # triplet style loss
            ...
            loss_pos = sum(self.style_loss(q, pos) for q, pos in zip(query, positive_key))
            loss_neg = sum(self.style_loss(q, neg) for q, neg in zip(query, negative_keys))
            return loss_pos - loss_neg
        else:
            query = [item[target_tensor == 0, ...] for item in input_tensor]
            positive_key = [item[target_tensor == 1, ...] for item in input_tensor]
            return sum(self.style_loss(q, pos) for q, pos in zip(query, positive_key))
