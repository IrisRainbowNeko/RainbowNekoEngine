...
class NekoDataLoader:
    def __init__(self, dataset: BaseDataset, batch_size=1, shuffle=False, sampler: Union[Sampler, Iterable, None] = None,
                 num_workers: int = 0, collate_fn: Optional[_collate_fn_t] = None, generator=None,
    ...
    @staticmethod
    def worker(worker_id, dataset, sampler, queue, queue_next, bs, prefetch_factor):
        """
        每个子进程执行的函数
        """
        batch = []
    ...
    @staticmethod
    def worker_iter(worker_id, dataset, queue, queue_next, bs, prefetch_factor):
        """
        每个子进程执行的函数
        """
        batch = []
    ...
    def multi_process_iterate(self, dataset, num_workers=4, bs=64):
        ...
        for worker_id in range(num_workers):
            if self.sampler == -1:
                p = ctx.Process(target=self.worker_iter, args=(worker_id, datasets[worker_id], queue,
                                                               queue_next_list[worker_id], bs // num_workers,
                                                               self.prefetch_factor))
            else:
                p = ctx.Process(target=self.worker, args=(worker_id, datasets[worker_id], self.sampler, queue,
